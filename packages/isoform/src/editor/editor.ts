...
import { buildChrome, type ChromeOptions } from './chrome.js'


export interface EditorOptions {
  /** Document to open with. Defaults to the sample diagram. */
  doc?: Doc
  /** Which pieces of chrome to build. */
  chrome?: ChromeOptions
  /**
   * Expose the internals on the returned handle as `debug`.
   *
   * Off by default. These are the live scene graph, reconciler and history —
   * useful for tests and instrumentation, and not a surface anything should
   * depend on.
   */
  debug?: boolean
}

/** Everything an embedder can do with a mounted editor. */
export interface Editor {
  /** The document as it stands. Structurally shared; treat as read-only. */
  readonly doc: Doc
  /** Replace the document, discarding undo history. */
  load(doc: Doc): void
  /** Serialisable snapshot of the current document. */
  save(): Doc
  /** Fires after any change, with the new document. Returns an unsubscribe. */
  onChange(fn: (doc: Doc) => void): () => void
  /** Re-frame the camera on the diagram. */
  fit(): void
  /** Render a PNG data URL of the diagram, without the editing scaffolding. */
  toPNG(width?: number): string
  /** Remove the editor's DOM, drop its listeners and stop its render loop. */
  destroy(): void
  /** Internals, when `debug` was set. */
  debug?: Record<string, unknown>
}

/**
 * Mount an editor into `container`.
 *
 * The body below is the original application script, wrapped. That is the point
 * of doing it this way: every `const` and `let` it declared at module scope
 * becomes closure state belonging to one editor, so two of them on a page do
 * not share a camera, a selection or a history — which a class conversion would
 * have had to reproduce by hand, statement by statement, across 1500 lines.
 */
export function createEditor(container: HTMLElement, options: EditorOptions = {}): Editor {
  const ui = buildChrome(container, options.chrome)

  /* Window-level listeners are tracked so `destroy` can lift them. An editor
     that keeps handling pointer moves after it has been unmounted is a leak
     that presents as a crash in whatever replaced it. */
  ...
  const onWindow = <K extends keyof WindowEventMap>(
    ...
    windowListeners.push([type, fn as EventListener])
  }

  /* Inspector contents are written as innerHTML and re-created on every
     selection, so its controls are still found by query — but within this
     editor's own panel, and keyed on `data-i` rather than `id`, so a second
     editor on the page cannot answer for the first. */
  const inspEl = <T extends HTMLElement>(key: string): T | null =>
    ui.inspBody.querySelector<T>(`[data-i="${key}"]`)

  const subscribers = new Set<(doc: Doc) => void>()
  ...
  const initial = options.doc ?? SEED_DOC
  seedIds(initial)
  ...
  function chromeInsets(): Insets {
    const rail = ui.rail.getBoundingClientRect()
    const insp = ui.insp.getBoundingClientRect()
  ...
  function setPresetButtons(active: CameraPreset): void {
    ...
    for (const k of ['hero', 'iso', 'top'] as const) {
      ui.view[k].setAttribute('aria-pressed', String(active === k))
    }
  }
  ...
  const undoBtn = ui.undo
  const redoBtn = ui.redo
  ...
  const inspTitle = ui.inspTitle
  const inspBody = ui.inspBody
  ...
  function renderGroupInspector(id: string): void {
    ...
    inspBody.innerHTML = `
      <div class="row"><label>Label</label><input type="text" data-i="i-glabel" value="${escapeHtml(g.label ?? '')}"></div>
      <div class="row"><label>Colour</label>
        <select data-i="i-gcat">
          ${GROUP_CATS.map(
      ...
      <div class="row"><button class="t" data-i="i-gadd">Add selection</button></div>
      <div class="row"><button class="t" data-i="i-gungroup">Ungroup</button>
        <button class="t" data-i="i-gdel">Delete</button></div>
      <p class="hint">Select parts, then reopen this panel to add them.
        <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>G</kbd> ungroups.</p>`

    const labelEl = inspEl('i-glabel') as HTMLInputElement
    labelEl.addEventListener('change', () => {
      run({ t: 'updateGroup', id, props: { label: labelEl.value || (null as never) } }, 'group label')
    })
    const catEl = inspEl('i-gcat') as HTMLSelectElement
    catEl.addEventListener('change', () => {
      run({ t: 'updateGroup', id, props: { cat: catEl.value as DocGroup['cat'] } }, 'group colour')
    })
    inspEl('i-gadd')!.addEventListener('click', () => {
      if (!pendingForGroup.length) return
      const next = [...new Set([...members, ...pendingForGroup])]
      setGroupMembers(id, next)
    })
    inspEl('i-gungroup')!.addEventListener('click', ungroupSelection)
    inspEl('i-gdel')!.addEventListener('click', () => {
  ...
  function renderInspector(): void {
    ...
    if (many.length > 1) {
      ...
      inspBody.innerHTML =
        ...
        `<div class="row"><label>Turn</label>
          <button class="t sq" data-i="i-rot-l" title="Rotate 15° left">↺</button>
          <button class="t sq" data-i="i-rot-r" title="Rotate 15° right">↻</button></div>` +
        `<div class="row"><button class="t" data-i="i-group">Group these</button></div>` +
        `<p class="hint">Or press <kbd>Ctrl</kbd>+<kbd>G</kbd>. Turning several parts
          swings them around their shared centre.</p>`
      inspEl('i-rot-l')!.addEventListener('click', () => rotateSelection(-ROT_STEP))
      inspEl('i-rot-r')!.addEventListener('click', () => rotateSelection(ROT_STEP))
      inspEl('i-group')!.addEventListener('click', groupSelection)
      return
    }
    ...
    inspBody.innerHTML = `
      <div class="row"><label>Label</label><input type="text" data-i="i-label" value="${n.label ?? ''}"></div>
      <div class="row"><label>Tint</label>
        <input type="color" data-i="i-tint" value="${n.tint ?? '#8b92ff'}">
        <button class="t" data-i="i-tint-reset">Reset</button></div>
      <div class="row"><label>Rot</label>
        <button class="t sq" data-i="i-rot-l" title="Rotate 15° left">↺</button>
        <input type="text" class="num mono" data-i="i-rot" value="${degrees(n.rot)}">
        <button class="t sq" data-i="i-rot-r" title="Rotate 15° right">↻</button></div>
  ...
  ui.fit.addEventListener('click', () => applyPreset(preset === 'free' ? 'hero' : preset))
  for (const k of ['hero', 'iso', 'top'] as const) {
    ui.view[k].addEventListener('click', () => applyPreset(k))
  }
  const gridBtn = ui.grid
  gridBtn.addEventListener('click', () => {
    const on = gridBtn.getAttribute('aria-pressed') !== 'true'
    gridBtn.setAttribute('aria-pressed', String(on))
    stage.setGridVisible(on)
  })
  const labelsBtn = ui.labels
  ...
  const dslText = ui.dslText
  const dslStatus = ui.dslStatus
  dslText.value = DSL_EXAMPLE

  ui.dslToggle.addEventListener('click', () => sheet.classList.toggle('hidden'))
  ui.dslClose.addEventListener('click', () => sheet.classList.add('hidden'))
  ui.dslApply.addEventListener('click', () => {
  ...
  const fileEl = ui.file
  ui.load.addEventListener('click', () => fileEl.click())
  ...
  const editor: Editor = {
    ...
    fit: () => applyPreset(preset === 'free' ? 'hero' : preset),
    toPNG: (width = 3840) =>
  ...
  if (options.debug) {
    editor.debug = {
      ...
      refreshDetail,
      /* Off-screen capture. A hidden browser pane does not composite, so a real
         screenshot comes back blank; rendering to a target and reading the
         pixels back is the only way to see what the scene looks like. */
