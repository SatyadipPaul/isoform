# Isoform — 3D System Designer

Machined architecture diagrams in the browser. Parts are modelled as the objects
they represent — a rack chassis, a shipping container, a bucket with a handle, a
brick wall — under one locked studio rig.

Grown from `3d-architecture-components_1.html`, a single-file three.js r128
component catalog. That file proved the visual language and shipped a reusable
substrate; this repo turns it into an engine.

```bash
npm install
npm run dev        # editor shell            → localhost:5173
...
| Package | What it is |
|---|---|
| `packages/engine` | `@isoform/engine` — foundry, part registry, document model, router, reconciler |
| `packages/editor` | `@isoform/editor` — the canvas-first editing application |

The engine is source-first: the editor imports its TypeScript through the
workspace, and `npm run build -w @isoform/engine` emits ESM + declarations to
`dist/` for publishing. Nothing in the engine imports editor code — that
boundary is the reason for the split.
